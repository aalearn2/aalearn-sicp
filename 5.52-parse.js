...
function parse(tokens) {
	...
	if (token_type == 'open-paren') {
		...
		var quote_list_end = [];
		var list_end = [];
		insert_points[insert_points.length-1].push(["'", quote_list_end]);
	    ...
	    insert_points[insert_points.length-1].push(parseInt(token[1]));
	} else {
	    if (quote_next) {
		insert_points[insert_points.length-1].push(token[1]);
		quote_next = false;
	    } else {
		insert_points[insert_points.length-1].push(token[1]);
	    }
